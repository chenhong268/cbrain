...
const CASES: AliasCase[] = [
  ...
  { alias: "job_list", aliasArgs: { status: "done" }, canonical: "job", canonicalArgs: { action: "list", status: "done" }, seed: (_root, db) => { db.rawDb.prepare("INSERT INTO jobs (name, status, priority) VALUES (?, ?, ?)").run("anonymous-list", "done", 1); } },
  { alias: "job_status", aliasArgs: { id: 1 }, canonical: "job", canonicalArgs: { action: "status", id: 1 }, seed: (_root, db) => { db.rawDb.prepare("INSERT INTO jobs (name, status, priority) VALUES (?, ?, ?)").run("anonymous-status", "pending", 1); } },
  { alias: "job_cancel", aliasArgs: { id: 1 }, canonical: "job", canonicalArgs: { action: "cancel", id: 1 }, seed: (_root, db) => { db.rawDb.prepare("INSERT INTO jobs (name, status, priority) VALUES (?, ?, ?)").run("anonymous-cancel", "pending", 1); } },
  { alias: "job_retry", aliasArgs: { id: 1 }, canonical: "job", canonicalArgs: { action: "retry", id: 1 }, seed: (_root, db) => { db.rawDb.prepare("INSERT INTO jobs (name, status, priority, attempts, max_attempts) VALUES (?, ?, ?, ?, ?)").run("anonymous-retry", "failed", 1, 1, 3); } },
