...
export function registerKnowledgeTools(server: McpServer, ctx: ToolContext): void {
  server.registerTool("add_knowledge", {
    ...
    inputSchema: {
      subject: z.string().min(1).describe("主体实体名称或 slug"),
      subject_type: z.string().optional().describe("主体类型（创建 stub 时用，如 person/company，默认 person）"),
      facts: z.array(z.object({
        field: z.string().describe("frontmatter 字段名（如 current_title, birthday）"),
        value: z.string().describe("字段值"),
      })).optional().describe("结构化属性"),
      relations: z.array(z.object({
        target: z.string().describe("目标实体名称或 slug"),
        target_type: z.string().optional().describe("目标类型"),
        relation: z.string().describe("关系类型（如 任职, 认识, 合作）"),
      })).optional().describe("图谱关系"),
      hierarchy: z.object({
        reports_to: z.string().describe("上级实体名称或 slug"),
        reports_to_type: z.string().optional(),
      }).optional().describe("汇报关系"),
      note: z.string().optional().describe("文本备注（追加到主体 body）"),
      evidence: z.string().optional().describe("来源证据文本"),
