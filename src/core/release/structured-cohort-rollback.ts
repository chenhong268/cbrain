...
export type RollbackDeps = {
  ...
  readHealth(): Promise<{ ok: boolean; output_boundary?: unknown }>;
...
function validTarget(target: RollbackTarget): boolean {
  ...
  if (!Array.isArray(target.programArguments) || target.programArguments.length < 3) return false;
  const entry = target.programArguments[0];
  if (typeof entry !== "string" || !entry.endsWith("/bin/cbrain-serve-http.sh")) return false;
  if (!target.programArguments.includes("serve") || !target.programArguments.includes("--http")) return false;
...
async function healthIsLegacy(deps: RollbackDeps): Promise<boolean> {
  try {
    const health = await deps.readHealth();
    return health.ok === true && health.output_boundary === "legacy";
  } catch {
    return false;
  }
}
...
export async function rollbackStructuredCohort(deps: RollbackDeps): Promise<RollbackResult> {
  ...
  try {
    ...
    if (target.mode === "legacy" && await healthIsLegacy(deps)) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      if (await healthIsLegacy(deps)) return success("rolled_back", true);
      if (attempt < 4) await deps.sleep(200);
    }
    return { schema_version: 1, status: "failed", code: "HEALTH_NOT_VERIFIED" };
  } finally {
    try { release(); } catch { /* closed result must survive lock cleanup failure */ }
  }
}
