...
function renderClaim(claim: TopicClaim): string {
  const label = KIND_LABELS[claim.kind] ?? "[观察]";
  return `- ${label} ${claim.text}（来源：\`${claim.sourceSlug}\`）`;
}

/** Render the derived topic Markdown body. The provenance disclaimer marks
 *  the page as a reading aid — never independent factual evidence.
 *
...
export function renderTopicBody(output: TopicModelOutput, sourceSlugs: string[]): string {
  const lines: string[] = [
    ...
    "## 主要观察",
    "",
    ...output.observations.map(renderClaim),
    "",
    "## 细节",
    "",
    ...(output.details.length > 0 ? output.details.map(renderClaim) : ["（无）"]),
    "",
    "## 分歧与开放问题",
    "",
    ...(output.open_questions.length > 0 ? output.open_questions.map(renderClaim) : ["（无）"]),
    "",
    "## 来源",
    "",
