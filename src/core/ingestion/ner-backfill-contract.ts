...
/** The single accepted identity grammar for frozen NER sources. */
...
export function parseZeroLinkRepairMarker(value: unknown): ZeroLinkRepairMarker | null {
  ...
  if (
    repair.name !== ZERO_LINK_REPAIR_NAME ||
    repair.version !== ZERO_LINK_REPAIR_VERSION ||
    typeof repair.batchId !== "string" ||
    !UUID_RE.test(repair.batchId)
  ) return null;
 ...
 * fields cannot disagree across queue, claim, and execution boundaries.
 */
export function parseFingerprintedNerJob(data: Record<string, unknown>): FingerprintedNerJob | null {
  if (
    (data.kind !== undefined && data.kind !== "ner") ||
  ...
  if (repair && rawSourceKind !== undefined && rawSourceKind !== repair.sourceKind) return null;
