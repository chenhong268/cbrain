...
program
  ...
  .action(async (opts) => {
    ...
    const zipName = `cbrain-backup-${ts}.zip`;
    const zipPath = join(outputDir, zipName);

    const vaultDir = config.vaultPath;
    const dbPath = config.dbPath;
    const lancePath = config.lancePath;

    console.log(`正在备份...`);
    const files: string[] = [dbPath];
    // Include vault
    files.push(vaultDir + "/.");
    // Include LanceDB if exists
    if (existsSync(lancePath)) files.push(lancePath + "/.");

    try {
      execSync(`cd "${resolve(vaultDir, "..")}" && zip -r "${zipPath}" ${files.map(f => `"${f}"`).join(" ")} 2>/dev/null`, { encoding: "utf-8" });
...
program
  ...
  .action(async (path, opts) => {
    ...
    if (!opts.force) {
      console.log(`⚠️  将用 ${zipPath} 覆盖当前 vault 和数据库。`);
      console.log("   当前数据将被替换。确认？(y/N)");
      process.exit(0);
    }

    console.log(`正在恢复...`);
    try {
      const targetDir = resolve(config.vaultPath, "..");
      execSync(`cd "${targetDir}" && unzip -o "${zipPath}"`, { encoding: "utf-8" });
      console.log("✅ 恢复完成。运行 cbrain sync 以重建索引。");
    } catch {
      console.error("恢复失败。请确认系统已安装 unzip 命令。");
      process.exit(1);
    }
  });
