...
- **实体丰富** — 人物和组织随提及次数自动升级
- **默认窄工具面** — 日常 Agent 用 `agent` profile（recall + 读写 + 图谱），不直接面对 92 个工具。Profile 是降噪音的人体工学边界，不是安全边界
...
> 日常 Agent 不必面对全部 92 个工具——走 [`agent` profile](#mcp-tool-profiles) 只暴露 recall / 读写 / 图谱等用户态工具。下面是完整清单（reference）。
...
CBrain ships 92 MCP tools, but a daily Agent shouldn't have to navigate all of them. Pick a **tool profile** to narrow the surface:
