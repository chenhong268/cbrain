...
- **工具**：`deep_recall(query, detail='normal')`
- **注意**：不是 query。自动做关系、时间线、dossier 富化，一步出完整画像。
- **追问**：拿到 slug 后可用 `expand_entity` 补充细节

### 2. 两人/两公司什么关系
- **触发**：XX和YY什么关系 / 怎么认识的 / 有什么联系 / 之间
- **工具**：`graph_query(mode='traverse', depth=2)` → 或直接 `deep_recall` 看关联部分
- **深度分析**：走 connect.md skill 做完整关系分析

### 3. 全景概览/总结
- **触发**：总结 / 梳理 / 全貌 / 概览 / overview / 帮我理一下
- **工具**：`summarize(slug, depth=1)` — 图遍历 + 结构化概览
- **注意**：不是 query。query 只返回片段，summarize 返回全局鸟瞰。

### 4. 结构化档案页
- **触发**：这个人的全貌 / 完整档案 / dossier / RAGmap / 详细档案
- **工具**：`dossier(slug)` — 生成结构化档案（基本信息 + 关系网络 + 时间线 + 洞察）
- **区别**：review 是叙事式总结，dossier 是结构化表格

### 5. 头脑风暴/分析推理
- **触发**：分析一下 / 联想 / 知识缺口 / cross-domain / 有什么盲点 / 帮我想想
- **工具**：`brain_storm(query)` — LLM 推理 + 缺口分析 + 跨域关联
- **注意**：不是 query。query 只检索已有知识，brain_storm 做推理和联想。

### 6. 快速查找
- **触发**：搜 / 找 / 有没有 / 查一下 / 谁在XX / XX认识谁
- **工具**：`query(query, limit=10)` — 混合搜索，返回 slug + title + snippet
- **后续**：找到后可 `expand_entity` 看详情

### 7. 展开实体/补充细节
- **触发**：多说说 / 展开 / 详细看这个 / 补充 / 丰富 / 完善
- **工具**：`expand_entity(slug)` — 单实体深度展开
- **前置**：需要先有 slug（通过 query/deep_recall/summarize 获取）
...
| 你想… | 用 | 不要用 |
|--------|-----|--------|
| 了解一个人/公司的全貌 | deep_recall | query（太浅） |
| 全局概览某个领域 | summarize | query（太零散） |
| 看两个人的关系 | graph_query 或 connect | query（没有关系推理） |
| 分析和推理 | brain_storm | query（只检索不推理） |
| 写内容 | write skill | 不要 Agent 瞎编，用 CBrain 知识 |
| 结构化档案 | dossier | review（叙事式 vs 结构化） |
...
❌ 无 slug 直接调 expand_entity → 先 query/deep_recall 拿 slug
❌ "帮我了解XX" 用 query → deep_recall
