---
name: cbrain
description: "Use when retrieving, organizing, or validating knowledge in CBrain."
---
...
凡用户询问“最新/发布/版本/价格/新闻/政策/行情/当前状态”等可能随时间变化的外部事实：
1. 仍先查 CBrain；它只用于私人上下文与既有判断。
2. CBrain 未命中、资料过期/低相关，或用户明确要求最新信息时，必须立刻检索外网，优先官方发布页、监管机构或一手数据源。
3. 不得把“CBrain 无记录”表述为“外部不存在/无法确认”；回答须区分 CBrain 记录与外部核验结果，并给出来源链接。
4. 外网主源不可访问时，改用浏览器或第二个独立来源；两者都失败才说明核验受阻。
