...
规则：
- `packVersion` 手写，verify 时强校验 `== src/version.ts` 导出的 version（即 `package.json` version）。不等 → canonical 自检 fail（exit 1）。这是"不静默漂移"守卫：release bump version 忘改 manifest 会被 `skill-pack` 和 docs gate 双重拦截。
...
- **最小强制集合 `ENTRY_FILES`**（硬编码 const，不在 manifest）：`SKILL.md`、`hermes-cbrain-brief.md`、`RESOLVER.md`、`recall-resolver.md`（`SKILL.md` §1 启动流程必读入口）。verify 时校验 `ENTRY_FILES ⊆ manifest.files[]`，否则 manifest 自残（删关键文件后仍通过）→ `MANIFEST_INVALID`。
- **MANIFEST.json 不进自己的 hash 列表**，单独验证（存在 + 可解析 + `packVersion` 匹配）。不自证。
...
| # | target.status | 条件 |
|:---:|:---|:---|
| 1 | `missing` | target 路径不存在，或明确为空目录 |
| 2 | `unverified` | canonical 自检 fail（无比对基准） |
| 3 | `incompatible` | target 目录存在但无 MANIFEST / MANIFEST 不可识别，或 `target.packVersion ≠ canonical.packVersion` |
| 4 | `stale` | version 相同，但 `files[]` 缺文件或某文件 hash ≠ canonical |
| 5 | `current` | version 相同 + 全文件 hash 匹配 |

关键语义修正（宏哥阻断点 1）：
- `missing` **严格** = 路径不存在 or 空目录。**只有 `missing` 才展示安装命令**。
...
**错误信封**（`--json` 失败时）：`{version, packPath, verificationStatus:"fail", status:"fail", code, error}`。稳定 code：`PACK_NOT_FOUND`（canonical skills dir 缺失）/ `PACK_INVALID` / `MANIFEST_MISSING`（canonical 无 MANIFEST）/ `MANIFEST_INVALID`（格式坏或 `ENTRY_FILES` 缺失）/ `VERSION_MISMATCH`（`packVersion ≠ version.ts`）。无 stack trace 泄露 stderr。
...
新增/扩展 check（宏哥阻断点 5）：
- **install-target 不嵌套**：扫描 install 文档里 skill-pack 的 copy/symlink 命令，断言 target 路径**不**以 `/skills/` 结尾（拦截嵌套退化）。
...
宏哥选定 Skill-layer fixture。operational→`next_actions` 在 TS router 无路（且非目标禁改），故 operational 路由只存于 skill 层。

- `agent-facing.routing-eval.jsonl` 加 3 类匿名 case（实现时读现有 jsonl schema 对齐格式）：
  - operational："系统当前有什么异常" / "接下来处理什么" → expected_tool=`next_actions`
  - natural：普通知识问题 → expected_tool=`cbrain_recall`
  - debug：精确 debug 关键词 → expected_tool=`query`
- bun:test 把 natural/debug 三类匿名 fixture 跑过 `classifyFrontdoorQuery` 断言（集中现有分散覆盖成 fixture；operational 因 TS router 无此路，不在 bun:test 跑 TS 分类）。
- operational 靠 doc-contract check 保证规则链存在：`RESOLVER.md`（当前异常/接下来→`query.md[operations]`）+ `query.md[operations]`→`next_actions({include_raw:false})`。扩展现有 `checkAgentWorkflowContract`。
- TS router **不动**。
...
1. `tests/cli/skill-pack.test.ts`（扩展现有 715 行）：
   ...
   - manifest 驱动 files（假第 34 文件进 manifest → 被 verify；假文件不在磁盘 → fail）
   - target 路径不存在 → `target.status=missing` + exit 1（非异常信封）
   - target 空目录 → `missing`
   - target 存在无 MANIFEST → `incompatible`
   - target version 不匹配 → `incompatible`
   - target version 等 + 文件改 → `stale`
   - target 全匹配 → `current`
...
2. `tests/bin/check-docs-consistency.*.test.ts`：
   - install-target `/skills/` 结尾 → fail
   - `MANIFEST.packVersion ≠ package` → fail
3. routing：三类匿名 fixture bun:test + jsonl case + doc-contract（operational 规则链）。
...
5 点修正全部纳入：`missing` 收紧、MANIFEST 全文件 + `ENTRY_FILES`、安装命令收紧、隐私边界允许操作路径、`verificationStatus`/`status` 语义 + packVersion gate + Hermes smoke 列为合并后 release gate。

## 6. 残余风险

- **`github:<owner>/cbrain` 硬编码**：install-onboarding.md 里功能性公开 install source（lines 31/55/445），非私人数据但隐私扫描可能命中。本 issue 不动，交宏哥定是否抽象为占位。
- **Hermes 真实加载行为未验证**：本 issue 只保证加载路径同源 + 匿名 fixture，真实 Hermes 是否读 SKILL.md 留合并后 release gate。若 Hermes 运行时确不读 SKILL.md，则部署合同的运行时价值取决于该 gate 结论。
- **routing-eval.jsonl schema 对齐**：加 operational category 是新 category，实现时须读现有 jsonl 格式确认字段兼容，避免破坏现有 29+ case 的外部消费。
