...
| `insight` | Unified insight lifecycle tool. |
...
| `next_actions` | 只读。返回当前最该处理的 top 1-3 项（跨健康/发现信号）。不修复、不删除、不合并、不提升候选。健康信号来自 run_action_candidates 已分类的候选，不重跑健康检查。 |
...
| `timeline` | Unified timeline operations. |
