...
> Current: `v2.0.19` — 精确人物生日查询的候选外兜底。
...
- 直接询问唯一精确标题人物的生日时，先走原有搜索与原始记录兜底；仍无结果才从该人物的原页面核实日期，避免小候选预算漏掉明确字段。
- 同名或别名归属不唯一时拒答；磁盘原页已有的身份字段若与索引冲突则拒答，同时保留字段不完整但同行明确写出主体生日的原始记录。

### Compatibility / Migration

- 不新增搜索调用、接口、数据库表、持久状态或迁移。回退至 v2.0.18 会恢复候选外正例漏召回；跨页日期冲突仍不在本次事实历史范围内。

## [v2.0.18] — 2026-09-24

### 生日查询正例准入（#520）

- 人物页已进入搜索候选、且原页面能核实同主体的有效生日时，不再因通用短语覆盖不足而返回空结果；派生主题页不作为生日原始证据。
- 保留对异主体、纠错、待核实和无效日期的拒绝。搜索候选之外的记录仍受既有检索限制。

### Compatibility / Migration

- 仅调整直接生日查询的现有内容准入，不新增搜索调用、接口、状态或数据迁移。回退至 v2.0.17 会恢复此正例漏召回，但此前误报修复仍在。

## [v2.0.17] — 2026-09-24

### 生日召回证据核验（#520）

- 内容前门的直接生日、出生日期查询仅放行与唯一人物主体绑定的明确日期；同页其他人物、生成关系、显式纠错或待核实值不能冒充答案。日期需符合日历规则，中文年/月/日不会被截短。
- 缺少可核实证据时明确说明，相关人员或工作记录不再被呈现为生日答案；有证据的次位 FTS 记录仍可进入结果。

### Compatibility / Migration

- 仅改变上述直接查询的内容召回准入；不新增工具、接口、数据库表、持久状态、依赖或后台进程。其他查询和路由保持原样。
- 无数据迁移。回退到 v2.0.16 只需回切代码与技能包，但会恢复该误报行为。发布后须以真实查询验收，未完成前不宣称生产修复。

## [v2.0.16] — 2026-09-13

### 主题数量有界扩展（#516）

- 管理主题页总量上限从 5 提升至 32：仅通过重复显式选择（每次请求仍最多 5 个 key）扩容，自动补齐仍封顶 5 个，不会创建未选择的主题；达到 32 后新种子显式阻断。
- 后续启用与既有待建选择合并，此前失败且已获授权的种子不会丢失；重复启用已建成的同一种子保持幂等（不重复建页、不报容量阻断、清理待建项）。
- 具名主题查找（每日概览/主题阅读路径）沿用共享总量上限，覆盖第 6 个及之后的主题；过期或人工修改的主题页仍被排除。五个试点数量、默认关闭与来源预算不变。

### 主题来源完整收录（#515）

- 主题维护不再把每个种子的来源静默截断为排序后的前 12 条：预览、创建、重试与刷新均使用完整成员关系，后加入排序靠后的记录会真正进入编译输入与主题清单，不再只触发目录重证明。
- 来源数安全上限提升为 128 条（编译与读取共用同一预算）；150,000 字符材料预算、单次模型调用与整主题拒绝语义不变，超限显式阻断而非部分发布。五个试点主题数量与默认关闭不变。

### 来源列表与维护可靠性（#515、#516、#517）

- 来源链接逐项分段，继续使用现有切块器，避免较长列表连成一个段落而被向量接口拒绝；保留全部可点击链接。
- 重复启用按已保存的主题身份维护，保留既有标题；主题低于新建门槛或被预览去重隐藏时仍可更新。失败保留待重试选择，成功后清理，满额恢复不会重复建页。
- 目录校验失败明确报告阻断；取消后的编译不会继续写入目录校验元数据。

### Compatibility / Migration

- 不新增 MCP tool、HTTP endpoint、CLI flag、依赖、数据库表或后台进程。主题维护仍每 30 分钟检查，输入不变不调用模型。
- 超过旧版 12 条来源或 5 个主题后，回退 v2.0.15 必须恢复相应主题文件、索引与配置；只回退代码无法正确读取扩展后的主题。上线前应完成备份和副本回滚验证，恢复时保护备份后新增的原始记录。

## [v2.0.15] — 2026-09-13

### 自动主题页（#509、#510、#511）

- 在 `brain/topics/` 将多条原始记录整理为带原文引用和可点击来源链接的主题页。区分观察、用户想法与待确认内容；原始记录保持不变。
- 通过现有 `job` 接口的 `topic-wiki` 任务预览、启用、刷新或停用，默认关闭。首批最多 5 个主题，每页至少 3 条来源才能新建，最多选用 12 条完整材料；超过预算不静默截断。启用后每 30 分钟检查，并在启动和每日进行核对；输入未变化不调用模型。
- 来源内容、更正状态、目录与生成结果经过校验后才可读取。来源变动、删除、索引未完成或人工修改会阻止旧主题进入召回；失败、取消和并发编辑保留原有内容。
- 日常主题阅读与概览可使用有效主题页；原文细节、时间核验与事实依据仍使用原始记录。生成主题不参与 NER，不作为独立证据，也不作为其他主题的来源。
- 来源目录校验避免逐条关系重复扫描记录，保持既有指纹和来源失效保护不变（#513）。

### Compatibility / Migration

- 不新增 MCP tool、HTTP endpoint、CLI flag、依赖或数据库表；主题清单保存在页面 frontmatter，启用状态复用现有 config。日常 Agent 工具数量不变。
- 初次启用前备份 SQLite、vault 和 LanceDB，并保留选择清单及执行回执。停用会停止自动维护，已有生成页仍保留。
- 启用后回退到旧版本前，必须先按回执撤销生成页及其索引和主题配置；只回退代码会失去派生来源的读取保护。恢复备份时应保留备份后新增的原始记录。

## [v2.0.14] — 2026-09-12

### 自然语言召回与回答依据

- 最近活动和明确标题的召回复用有界来源校验；修复时间、地点与主体线索遗漏，并保留命中文档内与问题相关的片段（#424、#453、#488、#505）。
- 检索失败不再冒充“没有记录”；官方 `deepseek-flash` 正确传递关闭思考的参数，校验超时会中止请求（#504、#505）。简单关系查询保持确定性路径，复杂关系问题仍走研究流程（#482）。
- 时间线排除生成关系和文件名日期误识别；前门查询日志保留真实命中来源，小结果集避免不必要的重排（#430、#445、#454、#480）。

### 写入、抽取与长期状态

- 原始文件保护覆盖显式写入及派生维护；同名页面、重命名和重复 slug 检查避免错误绑定，同一页面的索引更新串行化，失败后保持待恢复状态（#450、#451、#455、#456）。
- 恢复 deferred NER 与 watcher 配置传递，修复抽取超时分类、同名冲突、类型迁移引用和关系投影；推断组织层级不再写成权威事实（#459、#460、#462、#463、#465、#467、#487、#489）。
- 关系写入按 ontology 校验端点与别名冲突；Dream 共现学习保持幂等，已知关系投影不越界。备份恢复与真实 MCP 输入、召回、纠正后再召回合同得到回归验证（#408、#437、#444、#471、#472、#484）。

### 研究评估失败（#495）

- 充分性评估异常时保留已取得的证据，返回降级、低置信度与明确缺口，停止由失败评估驱动的补查，不再假定证据充分。

### Dream 清理失败报告（#497）

- 清理错误和依赖跳过进入进度、返回报告、简报及 Markdown，不再把失败等同于零项成功；任一页面清理失败时跳过依赖的向量孤儿清理。

### 研究时限与请求取消（#498）

- 研究规划和检索把取消信号传至聊天模型与向量请求；超时中止等待中的 HTTP、embedding 重试退避，迟到结果不能写入检索缓存或继续研究步骤。保留原有总预算、单次补查和普通请求重试策略。

### 有限补查的证据增益（#500）

- 现有单次补查跳过已尝试的等价动作，优先读取证据未覆盖的实体，并合并两轮执行覆盖；没有新动作或新证据时明确报告缺口和低置信度，前门直接显示停止原因；缺口不计入证据数，不增加模型调用或总预算。

### 任务取消与安全停止（#494）

- 取消任务后，迟到的完成、错误和进度不能覆盖取消状态；同进程 handler 收到取消信号，跨连接取消在安全边界检查。
- Dream 在阶段之间、NER 补抽在记录之间停止，保留已完成写入并释放 Dream 锁。取消任务仍不可 retry，需要重新提交；不改变 NER 租约或重试策略。
...
- CLI 和 dream 报告实际磁盘大小及增减，区分底层 prune 回收字节与净节省空间。失败恢复点不会被重试覆盖；本次不包含手工清理线上历史文件或备份。

### 页面过期时间（#476）

- 新建实体和启动迁移不再自动赋予 90 天期限。`expires_at` 只由明确输入设置，不根据实体类型或页面年龄推断；已有到期日原样保留。
- 过期查询和即将到期查询忽略空字符串及纯空格，明确日期的提醒行为保持不变。

### Compatibility / Migration

- 本版本不新增数据库表或 schema 版本；停止自动设置实体到期日，但不清理过去已保存的日期。安装不运行历史回填、sync 或数据修复；这些操作仍需独立执行。
- 维护版本保留期默认改为 6 小时，需要旧保留策略时可显式设置 `maintenance.compactRetentionHours: 168`。升级前保留独立的 SQLite、vault 和 LanceDB 备份；回滚代码不会恢复维护已回收的数据文件。
- MCP 接口和 legacy 输出默认保持不变。#412 人工对比已结案，决定不进入 structured cohort；#506 同步验收报告。Evidence–Claim–Validity 与任职时间校准仍为合同/影子验证，不代表新增生产时间查询能力。
- 可重新安装 `v2.0.13` 回滚代码；若升级后运行了维护、同步或修复，应按对应操作的备份与回滚方案恢复数据。

## [v2.0.13] — 2026-08-15

### 内容召回可靠性

- **已知中文业务记录不再被误判为空**（#413、#414）：当普通内容召回没有结果、同一页面至少有两段 FTS 证据、问题开头的主体与候选一致且该页面明显领先时，才返回该页的单一候选。主体不一致、证据不足或结果接近时仍保持空答，避免把模糊检索包装成事实。

### Compatibility / Migration

- 无数据库 schema、vault、FTS 或向量索引迁移；不执行 sync、重建或自动数据修复。既有 MCP tool、日常 Agent 20-tool profile 与 `CBRAIN_OUTPUT_BOUNDARY` 默认行为不变。
- 若需回滚，可重新安装上一稳定 tag `v2.0.12`；本版本不写入新的持久数据，因此无需数据回退。

## [v2.0.12] — 2026-08-15

### 写入与关系可靠性

- **空壳记录拒绝写入**（#376）：新建 `record` 页面若只有链接、frontmatter 或过短占位文字，会在写入 vault、SQLite、FTS 和向量索引前被确定性拒绝；普通合格记录与既有 record 更新保持原有行为。
- **任职关系只接受明确来源**（#375）：带明确 `manual` 或 `agent` 来源标记的组织字段，才会在严格实体消歧后投影为可信任的“任职”关系；NER 推断、歧义、错误类型和伪造来源均不会建边。
- **反思洞察同批去重**（#23）：一次 reflect 运行内，相同来源实体集合的洞察最多保留一条；临时写入失败不会阻止之后重试。

### 兼容性与安全

- **MCP 兼容合同可验证**（#377）：保留全部 26 个旧别名，同时用真实 MCP transport 验证旧入口和统一 action 的成功结果与写后状态一致；日常 Agent 仍固定为 20 个工具。`/health` 仅新增匿名的工具/配置清单数量，不返回名称或用户数据。
- **依赖 high advisory 修复**（#406）：将传递依赖 `js-yaml` 固定到已修复版本；发布前 dependency gate 继续阻断未处置的 critical/high 风险。

### Compatibility / Migration

- 无安装时的数据库 schema、vault、FTS 或向量索引迁移；安装本身不会自动清理或改写现有知识。之后若主动运行 `sync`，带明确 `manual` 或 `agent` 组织来源、且通过严格实体消歧的既有页面会补出对应的 SQLite 任职关系。
- 旧 MCP 别名继续可用；仓库文档改为推荐统一 action。若安装或启动验证失败，可重新安装上一稳定 tag `v2.0.11` 回退；回退不会删除已由 `sync` 补出的任职关系。
