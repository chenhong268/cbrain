...
> Current: `v1.8.10` — 新增被动情境找人能力，Hermes 可按时间、主题、场景或关系线索召回人物候选。

## [v1.8.10] — 2026-05-29

### Agent 体验
- **情境回忆重建（#10）**：新增 `recall_episode` MCP 工具，支持在用户忘记人名时，用时间段、主题、场景或共同关系召回可能的人物候选
- **证据收敛**：候选结果只返回匹配当前线索的 timeline/link evidence，避免把无关社会关系或经历带入回答
...
### 测试
- 新增情境召回单元测试与 MCP 集成测试，覆盖时间+主题、场景+关系、多候选不确定性、长期沉默关系、证据过滤和无主动联系建议
- `bun run check`：891 pass / 0 fail / 2207 expect() calls
